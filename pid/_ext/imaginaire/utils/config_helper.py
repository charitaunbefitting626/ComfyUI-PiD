...
def get_config_module(config_file: str) -> str:
    ...
    # Try resolving relative paths using directories in sys.path
    ...
    # Find which directory in sys.path contains the config file,
    # and convert the relative path into a dotted module name.
    config_module = None
    for path in sorted(sys.path, key=len, reverse=True):
        if not path:
            continue
        try:
            norm_sys_path = os.path.normpath(os.path.abspath(path))
            norm_config_file = os.path.normpath(os.path.abspath(config_file))
            if os.path.commonpath([norm_sys_path, norm_config_file]) == norm_sys_path:
                rel_path = os.path.relpath(norm_config_file, norm_sys_path)
                if not rel_path.startswith(".."):
                    config_module = rel_path.replace(os.sep, ".").replace(".py", "")
                    break
        except Exception:
            continue
            
    if config_module is None:
        # Fallback to simple replacement of both forward and backward slashes
        config_module = config_file.replace("\\", ".").replace("/", ".").replace(".py", "")
