...
class LocalBackend(BaseStorageBackend):
    ...
    def _resolve_pid_path(self, filepath: Union[str, Path]) -> Union[str, Path]:
        filepath_str = str(filepath)
        if os.path.exists(filepath_str):
            return filepath
        if not os.path.isabs(filepath_str):
            # Resolve relative to the PiD repository root
            # local_backend.py is at: pid/_ext/imaginaire/utils/easy_io/backends/local_backend.py
            pid_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "..", ".."))
            resolved = os.path.abspath(os.path.join(pid_root, filepath_str))
    ...
    def get(self, filepath: Union[str, Path]) -> bytes:
        ...
        Examples:
            ...
            b'hello world'
        """
        filepath = self._resolve_pid_path(filepath)
